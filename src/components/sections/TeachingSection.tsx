...
export function TeachingSection() {
  ...
  return (
    <section id="teaching" className="py-20 sm:py-28 lg:py-36 relative bg-[#06080e] text-white border-t border-white/15 atmosphere-emerald">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16 relative z-10">
        ...
        <div className="space-y-3 sm:space-y-4 mb-12 sm:mb-16 max-w-3xl">
          <div className="flex items-center gap-2 text-xs font-mono text-emerald-400 font-bold uppercase tracking-widest">
            <PixelTerminal size={16} color="#34d399" className="shrink-0" />
        ...
        {/* ---------------------------------------------------- */}
        <div className="mb-16 sm:mb-20 p-5 sm:p-8 lg:p-12 rounded-sm bg-black/85 border border-emerald-500/30 shadow-2xl relative overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
            ...
            <div className="lg:col-span-8 space-y-3 sm:space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/15 pb-3 sm:pb-4 font-mono text-xs gap-2">
                <div className="flex items-center gap-2 text-emerald-400 font-bold uppercase tracking-widest text-[11px] sm:text-xs">
                  <Compass className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span>{t("THE CORE TEACHING PHILOSOPHY", "TRIẾT LÝ GIẢNG DẠY CỐT LÕI")}</span>
                </div>
                <span className="text-slate-400 font-mono text-[10px] sm:text-[11px]">{t("100% INDEPENDENCE GOAL", "MỤC TIÊU 100% TỰ CHỦ")}</span>
              </div>

              <blockquote className="text-lg sm:text-2xl lg:text-3xl font-serif italic text-white leading-snug">
                &ldquo;{t(
            ...
            <div className="lg:col-span-4 flex items-center justify-center p-4 sm:p-6 bg-white/[0.02] border border-emerald-500/20 rounded-sm shadow-inner relative overflow-hidden">
        ...
        <div className="mb-16 sm:mb-24 space-y-4 sm:space-y-6">
          <div className="flex items-center justify-between border-b border-white/15 pb-3 sm:pb-4 font-mono text-xs">
            <span className="text-emerald-400 font-bold uppercase tracking-widest text-[11px] sm:text-xs">
              {t("// WHO THIS IS FOR", "// ĐỐI TƯỢNG PHÙ HỢP")}
            </span>
            <span className="text-slate-400 font-mono text-[10px] sm:text-[11px]">{t("BEGINNER TO ADVANCED BUILDER", "TỪ NGƯỜI MỚI ĐẾN BUILDER")}</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {teachingAudiences.map((aud, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-6 sm:p-8 bg-black/60 border border-emerald-500/20 hover:border-emerald-400/50 rounded-sm shadow-xl space-y-3.5 sm:space-y-4 transition-all group flex flex-col justify-between"
                >
                  <div className="space-y-2.5 sm:space-y-3">
                    <div className="flex items-center justify-between border-b border-white/15 pb-3">
                      ...
                      <span className="text-[9px] sm:text-[10px] font-mono tracking-widest px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 uppercase">
                        {badge}
                      </span>
                    </div>

                    <h3 className="text-lg sm:text-xl font-serif font-bold text-white group-hover:text-emerald-300 transition-colors">
                      {title}
                    </h3>
                  ...
                  <div className="pt-3 border-t border-white/10">
                    <div className="text-[9px] sm:text-[10px] font-mono text-emerald-400 font-bold uppercase tracking-wider mb-1">
        ...
        {/* ---------------------------------------------------- */}
        <div className="mb-16 sm:mb-24 p-5 sm:p-8 lg:p-12 bg-black/80 border border-emerald-500/30 rounded-sm shadow-2xl space-y-6 sm:space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/15 pb-3 sm:pb-4 gap-2 font-mono">
            <div className="text-xs text-emerald-400 font-bold uppercase tracking-widest text-[11px] sm:text-xs">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2.5 sm:gap-3 font-mono">
            {teachingData.learningFlow.map((step, idx) => {
              ...
              return (
                <div
                  key={step.step}
                  className="p-3 sm:p-4 bg-emerald-950/20 border border-emerald-500/20 rounded-sm flex flex-col justify-between min-h-[120px] sm:min-h-[140px] h-full relative group hover:border-emerald-400/50 transition-all"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-emerald-400 font-serif font-bold text-xs sm:text-sm">
                      {step.step}
                    </span>
                  ...
                  <div>
                    <div className="text-[11px] sm:text-xs font-bold text-white font-serif">{name}</div>
        ...
        <div className="mb-12 sm:mb-16 space-y-4 sm:space-y-6">
          <div className="flex items-center justify-between border-b border-white/15 pb-3 sm:pb-4 font-mono text-xs">
            <span className="text-emerald-400 font-bold uppercase tracking-widest flex items-center gap-2 text-[11px] sm:text-xs">
              <BookOpen className="w-4 h-4 text-emerald-400 shrink-0" />
              <span>{t("// PRACTICAL CURRICULUM MODULES", "// CÁC MODULE KIẾN THỨC THỰC CHIẾN")}</span>
            </span>
            <span className="text-slate-400 font-mono text-[10px] sm:text-[11px]">PROJECT-DRIVEN CURRICULUM</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {teachingData.curriculumTopics.map((topic, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="flex flex-col justify-between p-6 sm:p-8 bg-black/60 border border-emerald-500/20 hover:border-emerald-400/50 rounded-sm shadow-xl transition-all duration-300 group"
                >
                  <div className="space-y-3.5 sm:space-y-4">
                    <div className="flex items-center justify-between border-b border-white/15 pb-3">
                      <span className="text-lg sm:text-xl font-serif font-bold text-emerald-400 font-mono">
                        MOD 0{idx + 1}
                      </span>
                      <span className="text-[9px] sm:text-[10px] font-mono tracking-widest px-2 py-0.5 bg-emerald-500/10 text-emerald-300 uppercase">
                        PRACTICAL
                      </span>
                    </div>
                    ...
                    <div className="pt-2">
                      <span className="text-[9px] sm:text-[10px] font-mono text-emerald-400 font-bold uppercase tracking-wider block mb-1.5">
        ...
        <div className="p-6 sm:p-10 lg:p-12 rounded-sm bg-black/85 border border-emerald-500/30 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 sm:gap-8 shadow-2xl">
          <div className="space-y-2.5 sm:space-y-3 max-w-2xl">
            <span className="text-xs font-mono text-emerald-400 font-bold uppercase tracking-widest flex items-center gap-2 text-[11px] sm:text-xs">
          ...
          <Link
            href="#contact"
            className="w-full sm:w-auto px-6 sm:px-8 py-3.5 sm:py-4 bg-emerald-400 hover:bg-emerald-300 text-black font-mono text-xs font-bold uppercase tracking-widest transition-all shadow-xl shadow-emerald-500/20 rounded-sm shrink-0 flex items-center justify-center gap-2 touch-target text-center"
          >
