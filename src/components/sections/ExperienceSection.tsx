...
export function ExperienceSection() {
  ...
  return (
    <section id="experience" className="py-20 sm:py-28 lg:py-36 relative bg-[#070b14] text-white border-t border-white/15">
      <div className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-16">
        ...
        <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8 relative before:absolute before:left-3 sm:before:left-6 lg:before:left-8 before:top-4 before:bottom-4 before:w-[1px] before:bg-white/20">
          {experienceData.map((item, idx) => {
            ...
            return (
              <div key={item.id} className="relative pl-7 sm:pl-16 lg:pl-20 group">
                {/* Timeline node */}
                <div className="absolute left-1 sm:left-4 lg:left-6 top-5 sm:top-6 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-black border-2 border-amber-400 flex items-center justify-center shadow-lg group-hover:scale-125 transition-transform z-10">
                  <div className="w-1.5 h-1.5 rounded-full bg-amber-400" />
                </div>

                <div className="p-5 sm:p-8 lg:p-10 bg-black/60 border border-white/15 hover:border-amber-400/50 rounded-sm shadow-xl space-y-4 sm:space-y-5 font-sans transition-all">
                  {/* Header row */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 border-b border-white/15 pb-3 sm:pb-4">
                    <div>
                      <span className="text-xs font-mono text-amber-400 font-bold">
                        EPOCH N° 0{idx + 1}
                      </span>
                      <h3 className="text-lg sm:text-2xl font-serif font-bold text-white mt-0.5 sm:mt-1">
                        {role}
                      </h3>
                    ...
                    <div className="flex items-center gap-2 font-mono text-xs text-slate-300 self-start sm:self-auto">
                      <span className="px-2.5 sm:px-3 py-1 bg-white/5 border border-white/10 rounded-sm flex items-center gap-1.5 text-[10px] sm:text-xs">
                  ...
                  <div className="space-y-2 pt-1 sm:pt-2">
                    <div className="text-[9px] sm:text-[10px] font-mono font-bold uppercase tracking-widest text-amber-400">
                    ...
                    <ul className="space-y-1.5 sm:space-y-2 text-xs text-slate-300">
                      {responsibilities.map((resp, i) => (
                        <li key={i} className="flex items-start gap-2 sm:gap-2.5">
                          ...
                          <span className="leading-relaxed text-[11px] sm:text-xs">{resp}</span>
                        </li>
                      ))}
                    </ul>
                  </div>

                  <div className="flex flex-wrap gap-1.5 pt-2 sm:pt-3 font-mono">
                    {item.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-2 sm:px-2.5 py-0.5 sm:py-1 text-[10px] sm:text-[11px] bg-white/5 text-slate-300 border border-white/10"
                      >
                        {tech}
                      </span>
                    ))}
